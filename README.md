...
- 软件依赖：Python 3.10+、`ffmpeg`、`ffprobe`；macOS 已实机验证，Windows 由 GitHub Actions 的 `windows-latest` 持续验证，Linux 编码分支仍需单独验收。Windows 上如果 FFmpeg 版本不再提供 `filter_complex_script`，程序会自动改用文件参数读取滤镜图，必要时再以内联滤镜图兜底。
...
- macOS 默认使用 `h264_videotoolbox`；Windows 和 Linux 自动回退到 `libx264`。部分 Windows FFmpeg 版本不再提供 `filter_complex_script`，程序会自动把同一张滤镜图改为文件参数读取，必要时再改为内联参数重试。
